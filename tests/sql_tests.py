...
def run():
    ...
    check('select_alias_hides_table_name',
          ...
          message='column not found in schema')
    # JOIN syntax is represented in the statement-specific SELECT AST. Until
    # the two-table execution plan lands, binding rejects it explicitly rather
    # than silently executing only the left input.
    check('join_ast_plain',
          'SELECT u.id FROM users u JOIN aliases a ON u.age = a.i', rc=2,
          message='JOIN execution is not implemented yet')
    check('join_ast_inner',
          'SELECT u.id FROM users AS u INNER JOIN aliases AS a ON u.age = a.i',
          rc=2, message='JOIN execution is not implemented yet')
    check('join_ast_left',
          'SELECT u.id FROM users u LEFT JOIN aliases a ON u.age = a.i', rc=2,
          message='JOIN execution is not implemented yet')
    ...
    check('join_reversed_key_order',
          'SELECT u.id FROM users u JOIN aliases a ON a.i = u.age', rc=2,
          message='JOIN execution is not implemented yet')
    ...
    check('join_right_projection',
          'SELECT a.b FROM users u JOIN aliases a ON u.age = a.i', rc=2,
          message='JOIN execution is not implemented yet')
    check('join_mixed_projections',
          'SELECT u.id,a.b FROM users u JOIN aliases a ON u.age = a.i', rc=2,
          message='JOIN execution is not implemented yet')
