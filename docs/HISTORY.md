...
This is the development history of CybouDB, phase by phase, up to the first
public release `v0.5.0-preview.1` (2026-09-14). It is kept because the order
things were built in explains why the engine is shaped the way it is, and
because several phases record experiments that failed and the reason they
failed - the sort of thing a project usually throws away and then repeats.

It is **not** the plan. Planned work lives in [ROADMAP.md](../ROADMAP.md),
which after the first release is organised around releases and the guarantees
they make rather than around phases.

Checkboxes describe the repository as it stood: an item is ticked when it
exists in the code and is covered by the test suite.
...
See [the COW decision](COW.md). The existing allocator remains unsafe for
...
See [the PAX contract](PAX.md), [multi-page limits](PAX_MULTI.md)
and [the span map](SPAN_MAP.md).
...
everything it is not. The design is in [docs/TOMBSTONES.md](TOMBSTONES.md):
...
* [x] leaf capacity first, before the directory - see
      [docs/PAX_CAPACITY.md](PAX_CAPACITY.md). A leaf is now a run of
...
(see [docs/ZONEMAP.md](ZONEMAP.md)). The structure is validated and SQL
...
* [x] optional Compression V1: RAW/CONST/FOR in fixed-width PAX slots,
      ...
      regression tests. See [docs/COMPRESSION.md](COMPRESSION.md).
...
decisions are in [docs/INDEX.md](INDEX.md).
...
The decisions are fixed in [docs/QUEUE.md](QUEUE.md).
...
The decisions are fixed in [docs/STREAM.md](STREAM.md).
